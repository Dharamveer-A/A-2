    ...
    for i, sub_node in enumerate(subjects):
        ...
        def unenroll_button():
                if st.button("Unenroll from this course", type='tertiary', width='stretch', icon=':material/delete_forever:'):
                    ...
                    st.toast(f'Successfully unenrolled from {sub["name"]}.')
                    st.rerun()
...
def student_screen():
    ...
    if show_registration:
        with st.container(border=True):
            ...
            new_name = st.text_input("Enter your name", placeholder='E.g. Arjun Mehta')

            st.subheader("Optional: Voice Enrollment")
            st.info("Enrolling your voice enables voice-only attendance.")


            audio_data = None

            try:
                audio_data = st.audio_input('Record a short phrase like I am present, my name is Arjun.')
            except Exception:
                st.error('Unable to record audio.')
